...
public class PedidoDetalleDAO {
    ...
    public void eliminar2(PedidoDetalle pedidoDetalle) {
        ...
        if ( pedidoDetalle.getCantidad() > 1) {
             sql = "update pedido_detalle set cantidad = ? where id_pedido_detalle = ?";
            try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
                stmt.setInt(1, cantidad - 1);
                stmt.setInt(2, pedidoDetalle.getIdPedidoDetalle());
            ...
            try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
                stmt.setInt(1, pedidoDetalle.getIdPedidoDetalle());
    ...
    public PedidoDetalle buscarUnDetalle(int idpedido) {
        ...
        try (PreparedStatement stmt = cnx.prepareStatement(sql2)) {
            ...
            throw new RuntimeException("Error al Buscar  la Detalle del id ingresado", ex);
        }
        return detalle;
    }
}
